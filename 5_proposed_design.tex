...
\]
\noindent
Here, CI is connection interval, PPI is packets per interval, and BPP represents the number of bytes sent per packet. Because packets per interval and the bytes per packet may vary between devices 
...
- Application structure (class diagram?)	
  ...
  - Formats and websocket connection
...
Here well write a short introduction to the experiment. 
(...)
...
\subsubsection{Battery and throughput} % (fold)
\label{ssub:battery_and_throughput}

To be written.

% subsubsection battery_and_throughput (end)
